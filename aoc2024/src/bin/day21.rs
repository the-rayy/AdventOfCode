...
fn solve(input: &str, robots: u32) -> u64 {
    ...
    numpad_paths
        ...
        .map(|p| {
            ...
            p.windows(2)
                .map(|c| arrowpad(c[0], c[1], robots - 1, &mut cache))
                .sum()
        })
        .min()
        .unwrap()
...
fn numpad(chars: &str) -> HashSet<Vec<char>> {
    ...
    for c in chars.chars() {
        let dir = (
            numpad_position(c).0 - numpad_position(pos).0,
            numpad_position(c).1 - numpad_position(pos).1,
        );
        let permutations = dir_to_chars(dir);
        let new_moves = permutations
            .into_iter()
            .filter_map(|p| {
        ...
        options
            ...
            .map(|mv| {
                let mv = vec!['A']
                    ...
                    .collect::<Vec<char>>();
                mv.windows(2)
                    .map(|c| arrowpad(c[0], c[1], level - 1, cache))
                    .sum()
            })
            .min()
            .unwrap()
