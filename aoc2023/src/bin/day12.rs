...
fn count_possibilities(row: &str, nums: &Vec<u8>) -> usize {
    if row.contains("?") {
        ...
        return count_possibilities(row1.as_str(), nums) + count_possibilities(row2.as_str(), nums);
    }

    match valid(row, nums) {
        true => 1,
        false => 0
    }
}

fn valid(row: &str, nums: &Vec<u8>) -> bool {
    let actual_nums = row.split(".")
